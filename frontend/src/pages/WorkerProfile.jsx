...
function WorkerProfile() {
  ...
  if (isError || !worker) return <div className="container" style={{ padding: '3rem' }}><EmptyState icon={FaUserSlash} title="Target Context Missing" description="The professional metric you are attempting to trace could not securely resolve." /></div>;

  return (
    <div className="container" style={{ maxWidth: '900px' }}>
      <Link to="/workers" className="btn btn-ghost" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', marginBottom: '2rem' }}><FaArrowLeft /> Retract Query Focus</Link>
      ...
      <div className="card" style={{ padding: '3rem', position: 'relative', overflow: 'hidden', borderTop: 'none', border: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '2rem', position: 'relative', marginTop: '1.5rem', marginBottom: '2.5rem' }}>
          ...
          <div style={{ paddingBottom: '0.5rem' }}>
            <h1 style={{ margin: '0 0 0.5rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '2.25rem' }}>
              {worker.name} 
              {worker.verification_status === 'Verified' && <FaUserCheck color="#10b981" title="Security Background Analyzed Natively" size={28}/>}
            </h1>
        ...
        <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1.5rem', marginBottom: '4rem' }}>
           <div style={{ padding: '1.5rem', background: '#f8fafc', borderRadius: '12px', border: '1px solid var(--border)', textAlign: 'center' }}>
             ...
             <div style={{ fontSize: '1.75rem', fontWeight: 'bold', color: 'var(--text)' }}>{worker.averageRating || 'Uncalibrated'}</div>
             <div style={{ color: 'var(--text-light)', fontSize: '0.9rem', marginTop: '0.25rem' }}>Calculated Quality Ledger</div>
           </div>
           <div style={{ padding: '1.5rem', background: '#f8fafc', borderRadius: '12px', border: '1px solid var(--border)', textAlign: 'center' }}>
             ...
             <div style={{ color: 'var(--text-light)', fontSize: '0.9rem', marginTop: '0.25rem' }}>Master Trust Rating Output</div>
           </div>
           <div style={{ padding: '1.5rem', background: '#f8fafc', borderRadius: '12px', border: '1px solid var(--border)', textAlign: 'center' }}>
             ...
             <div style={{ color: 'var(--text-light)', fontSize: '0.9rem', marginTop: '0.25rem' }}>Reported Trade Tenure</div>
           </div>
           <div style={{ padding: '1.5rem', background: '#f8fafc', borderRadius: '12px', border: '1px solid var(--border)', textAlign: 'center' }}>
             ...
             <div style={{ color: 'var(--text-light)', fontSize: '0.9rem', marginTop: '0.25rem' }}>Physical Constraints</div>
           </div>
        </div>

        <h3 style={{ borderBottom: '1px solid var(--border)', paddingBottom: '1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>Client History Analysis</h3>
        {worker.reviews && worker.reviews.length > 0 ? (
            ...
            {worker.reviews.map((r, i) => (
              <li key={i} style={{ padding: '1.75rem', border: '1px solid var(--border)', borderRadius: '12px', marginBottom: '1rem', background: 'white' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem', alignItems: 'center' }}>
                  ...
                  <span style={{ color: '#f59e0b', fontWeight: 'bold', background: '#fef3c7', padding: '0.2rem 0.75rem', borderRadius: '12px' }}>⭐ {r.rating}/5 Format</span>
                </div>
          ...
          <EmptyState title="Metrics Absent" description="There have been zero historically verified logic tasks evaluated across this profile index." />
        )}

        <div style={{ marginTop: '3.5rem', textAlign: 'center' }}>
           <Link to={`/book/${worker.id}`} className="btn" style={{ padding: '1.25rem 3rem', fontSize: '1.25rem', display: 'inline-flex', alignItems: 'center', gap: '0.75rem', borderRadius: '8px', boxShadow: '0 4px 12px rgba(16, 185, 129, 0.3)' }}><FaCalendarAlt /> Dispatch Secure Booking Lock</Link>
        </div>
      </div>
    </div>
  );
